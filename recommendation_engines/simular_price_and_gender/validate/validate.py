...
from recommendation_engines.simular_price_and_gender import simular_gender, simular_price
...
def random_sampling_gender(sql_cursor, sample_type='random_samples'):
    """
    ...
    for product in products:
        result[product] = simular_gender.recommend_products(product, 4, sql_cursor)
    return result
...
def random_sampling_price(sql_cursor, sample_type='random_samples'):
    ...
    result = {}
    for product in products:
        result[product] = simular_price.recommend_products(product, 4, sql_cursor)
    return result
...
def take_mandatory_sampling_gender():
    ...
    random_products = random_sampling_gender(sql_cursor, 'mandatory_samples')
